...
- [ ] 1.1 T1 Protocol: PING builder + response parser; remove dead READ (~68 lines). Deps: none.
...
- [ ] 1.2 T2 Catalog `Nfc` bool + YAML round-trip (~33 lines). Deps: none.
...
- [ ] 2.1 T3 Client abstraction + serial client + fake (~124 lines). Deps: T1.
...
- [ ] 3.1 T4 `RetroBoxNfcWriter` (~110 lines). Deps: T1–T3.
...
- [ ] 4.1 T5 `nfc read`/`write` subcommands (~150 lines). Deps: T3–T4.
...
- [ ] 5.1 T6 Build + AOT smoke (0–10 lines). Deps: T1–T5. `mise run test` green; `mise run publish-linux-x64` (-warnaserror; note trim warnings); smoke `mise run cli -- --help` and `mise run cli -- nfc --help`.
